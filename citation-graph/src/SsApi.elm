...
type alias PaperInfo = { title: String
                       , authors : List String
                       , year : Int
                       , ssId: String
                       , isInfluential : Bool }
...
call s msg =
        Http.get { url = "https://api.semanticscholar.org/v1/paper/" ++ s ++
...
getConnection paper dic =
    let refList = List.map (\r -> r.ssId) paper.references in
    let citList = List.map (\c -> c.ssId) paper.citations in
    let mapSsid = IntDict.map (\i -> \p -> p.ssId) dic in
    let 
        mapRefSsid = IntDict.map 
                (\i -> \p -> ( List.map (\pi -> pi.ssId) p.references)) 
                dic
    in
    let 
        mapCitSsid = IntDict.map 
                (\i -> \p -> ( List.map (\pi -> pi.ssId) p.citations)) 
                dic
    in
    ( catSet (List.filterMap
...
findMap str l =
   case l of
      (i,s)::t -> if s == str then Just i else (findMap str t)
      [] -> Nothing

isIn : a -> List a -> Bool
isIn x l = List.foldl (\e -> \b -> if x == e then True else b) False l

findInMapList : List (Int, List String) -> String -> List Int
findInMapList lm s = List.filterMap (\(i, l) -> if isIn s l then Just i else Nothing) lm

catSet : List comparable -> List comparable -> List comparable
catSet l1 l2 = Set.toList (Set.fromList (l1 ++ l2))
